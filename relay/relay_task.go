...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	if adjustedRatios := adaptor.AdjustBillingOnSubmit(info, taskData); len(adjustedRatios) > 0 && !(info.PeakPricing != nil && service.IsTaskPerCallBilling(info)) {
...
func calculateTaskQuota(priceData hosttypes.PriceData) (int, *common.QuotaClamp) {
	charge := priceData.QuotaBeforeGroup * priceData.GroupRatioInfo.GroupRatio
	charge = priceData.ApplyOtherRatiosToFloat(charge)
	return common.QuotaFromPositiveFloatChecked(charge)
}
