...
// Orders retain their own status lock and idempotency check inside the wallet
// transaction. The owner lookup is only used to acquire locks in user-first order.
func withTopUpWalletTransaction(tradeNo string, work func(*gorm.DB) error) error {
	var order TopUp
	if err := DB.Select("user_id").Where("trade_no = ?", tradeNo).First(&order).Error; err != nil {
		return ErrTopUpNotFound
	}
	return withWalletTransaction(order.UserId, work)
}
...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	err = withTopUpWalletTransaction(referenceId, func(tx *gorm.DB) error {
